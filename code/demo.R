...
library(dplyr)
# setwd("./code")
setwd("~/Desktop/functional data analysis/code")
source("GeneData.R")

## the mfpca.face() function below is now in refund
## we load the script here just for demonstration purposes
source("mfpca.face.R")
source("face.Cov.mfpca.R") # a supplementary function, also in refund
...
for(iter in 1:nsim){
  ...
  for(i in 1:nrow(Y)){
    idx = which(!is.na(Y[i, ]))
    num = num + length(idx)
    diff1 = diff1 + sum(abs(fit_fast$Xhat[i,idx]-Y[i,idx])^2)
  }
  MISE1_Y <- diff1/num
  ...
  for(i in 1:nrow(Y)){
    idx <- which(!is.na(Y[i, ]))
    num <- num + length(idx)
    diff2 <- diff2 + sum(abs(fit_fast2$Xhat[i,idx]-Y[i,idx])^2)
  }
  MISE2_Y <- diff2/num
  # MISE of eigenfucntions
